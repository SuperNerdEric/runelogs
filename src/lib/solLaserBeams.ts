...
export const SOL_LASER_ALL_IDS = new Set([...SOL_LASER_SCAN_IDS, ...SOL_LASER_SHOT_IDS]);
...
function phaseFromId(id: number): SolLaserPhase {
    return SOL_LASER_SHOT_IDS.has(id) ? 'shot' : 'scan';
}

function textureIdForBeam(phase: SolLaserPhase, orientation: SolLaserOrientation): number {
    if (phase === 'shot') {
...
function beamFromPrism(prismX: number, prismY: number, spawnTick: number, phase: SolLaserPhase, plane: number): SolLaserBeam | null {
...
function groupLine(
    ...
    for (const [spawnTick, spawnTiles] of bySpawnTick) {
        const phase = spawnTiles.some((tile) => SOL_LASER_SHOT_IDS.has(tile.id)) ? 'shot' : 'scan';
        ...
        for (const tile of spawnTiles) {
            const fixedCoord = orientation === 'horizontal' ? tile.position.y : tile.position.x;
...
export function computeSolLaserBeams(
    ...
    const laserTiles = Object.values(graphicsObjects).filter((object) => isSolLaserGraphic(object.id));
    ...
    for (const beam of [...horizontalBeams, ...verticalBeams]) {
        ...
        if (!existing || beam.endVar - beam.startVar > existing.endVar - existing.startVar) {
