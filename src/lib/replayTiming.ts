...
export const CLIENT_CYCLE_DURATION_SECONDS = TICK_DURATION_SECONDS / CLIENT_CYCLES_PER_TICK;

export function getAbsoluteTick(currentTimeSeconds: number, initialTick: number): number {
...
export function computeFightEpochCycle(logs: LogLine[], initialTick: number): number | undefined {
    ...
    for (const log of logs) {
        ...
        const candidate = spawnLog.startCycle - (tick - initialTick) * CLIENT_CYCLES_PER_TICK;
...
export function getCurrentGameCycle(currentTimeSeconds: number, fightEpochCycle: number): number {
    return fightEpochCycle + Math.round(currentTimeSeconds / CLIENT_CYCLE_DURATION_SECONDS);
}
...
export function getGraphicObjectAnimationCyclesElapsed(
    ...
    if (timing.startCycle != null && fightEpochCycle != null) {
        return getCurrentGameCycle(currentTimeSeconds, fightEpochCycle) - timing.startCycle;
    }

    const ticksSinceSpawn = getAbsoluteTick(currentTimeSeconds, initialTick) - timing.spawnTick;
...
export function isGraphicObjectVisible(
    ...
    if (timing.endCycle != null && fightEpochCycle != null) {
        return getCurrentGameCycle(currentTimeSeconds, fightEpochCycle) < timing.endCycle;
    }
