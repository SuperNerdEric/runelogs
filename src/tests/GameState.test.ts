...
describe('getGameStateAtTick', () => {
    const fight = makeFight([
        {type: LogTypes.POSITION, tick: 5, source: {name: 'Player', id: 0, index: 0}, position: {x: 1, y: 2, plane: 0}},
        {type: LogTypes.POSITION, tick: 8, source: {name: 'Player', id: 0, index: 0}, position: {x: 3, y: 4, plane: 0}},
    ]);
    const gameStates = createGameStates(fight);
    ...
    it('maps replay time to the same tick state', () => {
        ...
        expect(getGameStateAtTick(gameStates, getTargetTick(currentTime, initialTick))?.tick).toBe(5);
    });
});
