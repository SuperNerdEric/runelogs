...
import {
  ...
  STANDALONE_LEADERBOARD_CONTENT_NAMES,
...
describe("leaderboard player count filters", () => {
  it("list only specific player counts because leaderboards are party-size specific", () => {
    for (const content of LEADERBOARD_CONTENT_OPTIONS) {
      ...
      expect(
        options.some((option) => option.value === BROWSE_ANY_PLAYER_COUNT),
      ).toBe(false);
      expect(options.map((option) => option.value)).toEqual([
        ...content.playerCounts,
      ]);
    }
  });
});
...
describe("Yama leaderboard content", () => {
  it("includes solo and duo player counts", () => {
    const yama = LEADERBOARD_CONTENT_OPTIONS.find(
      (option) => option.value === "Yama",
    );
    expect(yama).toEqual({
      label: "Yama",
      value: "Yama",
...
describe("Maggot King leaderboard content", () => {
  it("is solo-only", () => {
    const maggotKing = LEADERBOARD_CONTENT_OPTIONS.find(
      (option) => option.value === "Maggot King",
    );
    expect(maggotKing).toEqual({
      label: "Maggot King",
      value: "Maggot King",
...
describe("standalone leaderboard content", () => {
  it("lists Yama and Maggot King as standalone bosses", () => {
    expect(STANDALONE_LEADERBOARD_CONTENT_NAMES).toEqual([
      "Yama",
      "Maggot King",
    ]);
  });

  it("identifies standalone bosses and excludes fight groups", () => {
    expect(isStandaloneLeaderboardContent("Yama")).toBe(true);
    expect(isStandaloneLeaderboardContent("Maggot King")).toBe(true);
    expect(isStandaloneLeaderboardContent("Theatre of Blood")).toBe(false);
    expect(isStandaloneLeaderboardContent(null)).toBe(false);
  });
});
...
describe("buildPlayerRankLeaderboardHref", () => {
  it("links deep delve rank badges to the high-score leaderboard", () => {
    expect(
      ...
      "/leaderboards?mode=high-score&leaderboard=Doom+of+Mokhaiotl&playerCount=1&highlightRank=4",
    );
  });
  ...
  it("links delve DPS rank badges to the correct fight leaderboard", () => {
    expect(
      ...
      "/leaderboards?mode=dps&leaderboard=Doom+of+Mokhaiotl&playerCount=1&fight=Delve+1&highlightRank=3",
    );
  });
});
