...
function makeFight(overrides: Partial<Fight> = {}): Fight {
    return {
        ...
        logVersion: '1.3.6',
        firstLine: {type: LogTypes.DAMAGE, tick: 100, fightTimeMs: 0} as Fight['firstLine'],
        lastLine: {type: LogTypes.DEATH, tick: 110, fightTimeMs: 0} as Fight['lastLine'],
        ...overrides,
    };
}
...
describe('getFightPerformanceByPlayer', () => {
    it('does not produce negative activity when lastLine.fightTimeMs was zeroed by a phase transition', () => {
        ...
        const activityPercent = (performance.activeTicks / fight.metaData.fightDurationTicks) * 100;
    ...
    it('caps active time at the fight end when lastLine.fightTimeMs is valid', () => {
        const fight = makeFight({
            ...
            lastLine: {type: LogTypes.DEATH, tick: 110, fightTimeMs: 6000} as Fight['lastLine'],
            data: [
                {
                    type: LogTypes.PLAYER_EQUIPMENT,
                    tick: 100,
