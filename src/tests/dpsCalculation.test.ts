...
function makeFight(overrides: Partial<Fight> = {}): Fight {
    return {
        ...
        logVersion: '1.3.6',
        firstLine: {type: LogTypes.WAVE_START, tick: 100, fightTimeMs: 0} as Fight['firstLine'],
        lastLine: {type: LogTypes.WAVE_END, tick: 200, fightTimeMs: 60000} as Fight['lastLine'],
        ...overrides,
    };
}
...
describe('getFightDurationSeconds', () => {
    it('prefers tick duration over fightTimeMs', () => {
        const fight = makeFight({
            metaData: {name: 'Test Boss', startTime: '2025-01-01T00:00:00.000Z', fightDurationTicks: 100, success: true},
            firstLine: {type: LogTypes.WAVE_START, tick: 100, fightTimeMs: 0} as Fight['firstLine'],
            lastLine: {type: LogTypes.WAVE_END, tick: 200, fightTimeMs: 50000} as Fight['lastLine'],
        });
...
describe('calculatePlayerDps', () => {
    it('matches backend leaderboard rules', () => {
        ...
        expect(results.find((entry) => entry.playerName === 'Alice')).toMatchObject({damageDealt: 600, dps: 10});
        expect(results.find((entry) => entry.playerName === 'Bob')).toMatchObject({damageDealt: 300, dps: 5});
    });
    ...
    it('respects additional UI filters passed via logs', () => {
        ...
        const filteredLogs = fight.data.filter((log) => log.type === LogTypes.DAMAGE && (log as {source?: {name?: string}}).source?.name === 'Alice');
