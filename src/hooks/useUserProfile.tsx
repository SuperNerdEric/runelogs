...
export function UserProfileProvider({children}: {children: React.ReactNode}) {
    ...
    const setAvatar = useCallback(
        async (avatarId: AvatarId) => {
            const token = await getAccessTokenSilently();
            const resp = await fetch(`${import.meta.env.VITE_API_URL}/profile/avatar`, {
                method: 'PATCH',
                headers: {
    ...
    return (
        <UserProfileContext.Provider value={{profile, loading, error, fetchProfile, setAvatar, updateProfileDetails}}>
            {children}
