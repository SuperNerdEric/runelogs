...
function BlogEntry({post}: {post: BlogPost}) {
    ...
    const hasMore =
        post.body.paragraphs.length > 1 ||
        (post.body.bullets?.length ?? 0) > 0;
    ...
    return (
        ...
        >
            ...
            >
                ...
                <Link component={RouterLink} to={getBlogPostHref(post.slug)} sx={titleLinkSx}>
...
const Blog: React.FC = () => {
    ...
    const combatLoggerCount = BLOG_POSTS_SORTED.filter((p) => p.category === 'combat-logger').length;

    const runelogsCount = BLOG_POSTS_SORTED.filter((p) => p.category === 'runelogs').length;
