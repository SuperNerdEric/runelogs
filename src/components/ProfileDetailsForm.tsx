...
function getContactsFromProfile(
  profile: UserProfile,
): Record<ContactLinkKey, string> {
  return {
...
const ProfileDetailsForm: React.FC<ProfileDetailsFormProps> = ({
  ...
  const [contacts, setContacts] = useState<Record<ContactLinkKey, string>>(
    getContactsFromProfile(profile),
  );
  ...
  const contactsDirty = CONTACT_FIELDS.some(
    (field) =>
      contacts[field.key] !==
      getContactFormValue(field.key, profile[field.key]),
  );
  ...
  const handleSaveContacts = async () => {
    ...
    try {
      ...
      setContactError(
        err instanceof Error ? err.message : "Failed to save contact links",
      );
    } finally {
