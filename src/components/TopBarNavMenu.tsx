...
const TopBarNavMenu: React.FC<TopBarNavMenuProps> = ({
  iconButtonSx,
  onOpenChange,
}) => {
  const [open, setOpen] = useState(false);
  const [leaderboardsExpanded, setLeaderboardsExpanded] = useState(false);
  const [recentEncountersExpanded, setRecentEncountersExpanded] =
    useState(false);
  ...
  const recentEncountersLinks = RECENT_ENCOUNTERS_CONTENT_OPTIONS.map(
    (option) => ({
      label: option.label,
      value: option.value,
      ...
      >
        <Box
          sx={{
            ...
            height: "100%",
            bgcolor: colors.background.black,
          }}
        >
          <Box
            sx={{
          ...
          <List sx={{ py: 0.5, flex: 1 }}>
            ...
            <Collapse in={leaderboardsExpanded} timeout="auto" unmountOnExit>
              <List
                disablePadding
                sx={createSubmenuListSx(NAV_SECTION_LINE_COLORS.leaderboards)}
              >
            ...
            <Collapse
              in={recentEncountersExpanded}
              timeout="auto"
              unmountOnExit
            >
              <List
                disablePadding
                sx={createSubmenuListSx(
                  NAV_SECTION_LINE_COLORS.recentEncounters,
                )}
              >
            ...
            <Collapse in={communityExpanded} timeout="auto" unmountOnExit>
              <List
                disablePadding
                sx={createSubmenuListSx(NAV_SECTION_LINE_COLORS.community)}
              >
