...
const Combobox = <T extends ComboboxItem>(props: IComboboxProps<T>) => {
  const { id, items, onSelectedItemChange, placeholder } = props;
  ...
  const filteredItems = useMemo(() => {
    ...
    if (inputValue) {
      const newFilteredItems = fuzzyFilter(preprocessedItems, inputValue, {
        fields: ["valueToFilter"],
          ...
          <div style={{ height: listHeight, maxHeight: "500px", width: 250 }}>
            <Virtuoso
              ...
              itemContent={(i, d) => {
                return (
                  <div
                    className={`${highlightedIndex === i ? "combobox-hover" : ""}`}
                    {...getItemProps({
                      index: i,
                      item: d,
                    })}
