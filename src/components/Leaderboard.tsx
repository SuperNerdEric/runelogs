...
import {
  mapContentFilterOptions,
  mapFightFilterOptions,
} from "../utils/contentFilterOptions";
...
const Leaderboard: React.FC<LeaderboardProps> = ({ entriesPerPage = 25 }) => {
  ...
  const { mode, content, playerCount, selectedFight, highlightRank } = useMemo(
    () => resolveLeaderboardStateFromSearchParams(searchParams, dpsConfig),
    [searchParams, dpsConfig],
  );

  const [durationEntries, setDurationEntries] = useState<
    DurationEntry[] | null
  >(null);
  const [durationResultType, setDurationResultType] = useState<
    "fight" | "fightGroup"
  >("fight");
  ...
  const availableFights =
    dpsConfig.find((group) => group.contentName === content.value)?.fights ??
    [];
  ...
  const pageQuery = `&page=${page}&limit=${entriesPerPage}`;
  ...
  const fetchHighScoreData = useCallback(async () => {
    const url = `${import.meta.env.VITE_API_URL}/high-score-leaderboard?content=${encodeURIComponent(
      content.value,
    )}&playerCount=${playerCount}${pageQuery}`;
  ...
  const fetchDpsData = useCallback(async () => {
    const url = `${import.meta.env.VITE_API_URL}/dps-leaderboard?content=${encodeURIComponent(
      content.value,
    )}&fight=${encodeURIComponent(selectedFight)}&playerCount=${playerCount}${pageQuery}`;
    const res = await fetch(url);
    if (!res.ok) throw new Error(`Server returned ${res.status}`);
    const data = await res.json();
    setDpsEntries(data.leaderboard);
    setTotal(data.total ?? data.leaderboard.length);
  }, [content, playerCount, selectedFight, pageQuery]);
                    ...
                    <TableCell sx={{ color: rankColor, fontWeight: "bold" }}>
                      ...
                      >
                        ...
                        >
                          ...
                          {actualRank === 2 && (
                            <MedalIcon color={colors.medal.silver} />
                          )}
                          {actualRank === 3 && (
                            <MedalIcon color={colors.medal.bronze} />
                          )}
                        </Box>
                      </Link>
                    </TableCell>
              ...
              dpsRows.map((row) => {
                const percentile =
                  dpsTotal <= 1
                    ? 100
                    : Math.round(
                        ((dpsTotal - row.rank) / (dpsTotal - 1)) * 100,
                      );
      ...
      <FilterToolbar
          ...
          <>
            <FilterSelect
              ...
              onChange={(nextContentValue) => {
                ...
                const nextFight =
                  isMokhaiotlLeaderboardContent(selectedContent.value) &&
                  fights.includes(MOKHAIOTL_DELVE_1_8_KEY)
                    ? MOKHAIOTL_DELVE_1_8_KEY
                    : fights.includes("Overall")
                      ? "Overall"
                      : (fights[0] ?? "Overall");
                const nextMode = getLeaderboardModesForContent(
        ...
        modeSelector={
          <DurationDpsModeSelector
            ...
            onChange={(nextMode) => {
              const fights =
                dpsConfig.find((group) => group.contentName === content.value)
                  ?.fights ?? [];
              const nextFight =
                isMokhaiotlLeaderboardContent(content.value) &&
                fights.includes(MOKHAIOTL_DELVE_1_8_KEY)
                  ? MOKHAIOTL_DELVE_1_8_KEY
                  : selectedFight;
              setLeaderboardSearchParams({
        ...
        trailingFilters={
          ...
          !isStandaloneLeaderboardContent(content.value) ? (
            <FilterSelect
              field="fight"
              value={selectedFight}
