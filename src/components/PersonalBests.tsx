...
const PersonalBests: React.FC = () => {
  ...
  const [durationData, setDurationData] =
    useState<PersonalBestsResponse | null>(null);
    ...
    const fetchDuration = async () => {
      ...
      try {
        const res = await fetch(
          `${import.meta.env.VITE_API_URL}/player/${playerName}/personal-bests`,
        );
        if (!res.ok) {
  ...
  useEffect(() => {
    ...
    const fetchDps = async () => {
      ...
      try {
        const res = await fetch(
          `${import.meta.env.VITE_API_URL}/player/${playerName}/dps-personal-bests`,
        );
        if (!res.ok) {
          throw new Error(`Server returned ${res.status}`);
        }
  ...
  const getSelectedFight = (
    contentValue: string,
    availableFights: string[],
  ) => {
  ...
  const renderDate = (startTime?: string) => {
    ...
    return (
      <>
        ...
        <Box
          component="span"
          sx={{ display: "none", [media.desktopUp]: { display: "inline" } }}
        >
  ...
  return (
    <Box mt={4}>
      ...
      {!isBusy &&
        mode === "time" &&
        LEADERBOARD_CONTENT_OPTIONS.map((content) => {
                ...
                <Table>
                  ...
                  <TableBody>
                    {content.playerCounts.map((count) => {
                      const match = relevantFights.find(
                        (f) => f.playerCount === count,
                      );
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell sx={{ color: "white" }}>
                            {match
                              ? ticksToTime(match.officialDurationTicks)
                              : "-"}
                          </TableCell>
                          <TableCell sx={{ color: "white" }}>
      ...
      {!isBusy &&
        ...
        LEADERBOARD_CONTENT_OPTIONS.map((content) => {
          ...
          const contentEntries = dpsEntries.filter(
            (entry) => entry.contentName === content.value,
          );
          ...
          const availableFights =
            configFights.length > 0
              ? configFights.filter((fight) =>
                  contentEntries.some((entry) => entry.fightKey === fight),
                )
              : Array.from(
                  new Set(contentEntries.map((entry) => entry.fightKey)),
                ).sort();

          const selectedFight = getSelectedFight(
            content.value,
            availableFights,
          );
            ...
            <Box key={content.value} mb={3}>
              ...
              <TableContainer>
                <Table>
                  ...
                  <TableBody>
                    {content.playerCounts.map((count) => {
                      const match = relevantEntries.find(
                        (entry) => entry.playerCount === count,
                      );
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell
                            sx={{ color: rankColor, fontWeight: "bold" }}
                          >
                            {match ? match.dps : "-"}
                          </TableCell>
                          <TableCell
