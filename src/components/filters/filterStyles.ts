...
const radius = {xs: FILTER_TOOLBAR_RADIUS_MOBILE, sm: FILTER_TOOLBAR_RADIUS} as const;
const controlHeight = {xs: FILTER_CONTROL_HEIGHT_MOBILE, sm: FILTER_CONTROL_HEIGHT} as const;
const controlFontSize = {xs: FILTER_CONTROL_FONT_SIZE_MOBILE, sm: FILTER_CONTROL_FONT_SIZE} as const;
...
const FILTER_CONTROL_SURFACE_GRADIENT_FOCUS =
    `radial-gradient(ellipse 110% 100% at 50% 50%, rgba(56, 139, 253, 0.12) 48%, ${FILTER_TOOLBAR_SURFACE} 100%)`;
...
export const filterToolbarFiltersRowSx = (fullWidthOnMobile = false): SxProps<Theme> => ({
    ...filterFieldsRowSx,
...
export const filterToolbarDividerSx: SxProps<Theme> = {
    ...
    flexShrink: 0,
    width: '1px',
    height: {xs: FILTER_CONTROL_HEIGHT_MOBILE - 14, sm: FILTER_CONTROL_HEIGHT - 16},
    borderRadius: '999px',
...
export const filterModeTabStateSx = (selected: boolean): SystemStyleObject<Theme> => ({
    ...
    '&:hover': {
        background: selected ? FILTER_CONTROL_SURFACE_GRADIENT_HOVER : 'rgba(255, 255, 255, 0.04)',
        color: colors.text.primary,
    },
    '&.Mui-selected:hover': {
        color: colors.text.primary,
