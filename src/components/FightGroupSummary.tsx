...
const TOP_RANK_CATEGORIES = new Set([
  "Duration",
  "Overall DPS",
...
const FightGroupSummary: React.FC = () => {
      ...
      const execute = async () => {
        ...
        try {
          const res = await fetch(
            `${import.meta.env.VITE_API_URL}/fightGroup/${id}`,
          );
          if (res.status === 410) {
  ...
  const topRankBadges = useMemo(() => {
    ...
    return data.playerRanks.filter(
      (entry) =>
        TOP_RANK_CATEGORIES.has(entry.category) &&
        !isUnknownPlayer(entry.playerId),
    );
  }, [data]);
  ...
  const topRankLabel = (entry: PlayerRank) => {
    if (
      ...
      entry.category === MOKHAIOTL_HIGH_SCORE_MODE_LABEL
    ) {
      return "";
    }
    ...
    <Box p={2} sx={contentColumnSx} className="fight-group-summary">
      ...
      <PageBreadcrumbs
        segments={[
          ...
          {
            label: data.name,
            spriteKey: resolveFightGroupSpriteKey(
              data.name,
          ...
          fights={data.fights.map((fight, fightGroupIndex) => {
            ...
            const rankBadges = (fightRankBadgesByKey.get(fightKey) ?? [])
              ...
              .map((entry) => ({
                ...
                rank: entry.rank,
                percentile: resolvePlayerRankPercentile(
                  entry,
                  percentileContext,
                ),
              }));
