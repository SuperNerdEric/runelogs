...
const PlayerSearch: React.FC<PlayerSearchProps> = ({
  ...
  useEffect(() => {
    ...
    timeoutRef.current = setTimeout(() => {
      fetch(
        `${import.meta.env.VITE_API_URL}/player-search/${encodeURIComponent(input)}`,
      )
        .then((res) => res.json())
        .then((data) => {
  ...
  return (
    ...
    >
      ...
      >
        <Paper
          sx={{
            backgroundColor: colors.background.surfaceDropdown,
          ...
          <List>
            {results.map((name, idx) => (
              <ListItem
                ...
                sx={{
                  "&.Mui-selected": {
                    backgroundColor: colors.background.surfaceSelected,
                  },
