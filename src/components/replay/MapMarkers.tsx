...
import {
  graphicObjectIdMap,
  getGraphicObjectFrameIndex,
  getGraphicObjectTotalCycles,
} from "../../lib/graphicObjectIdMap";
import {
  gameObjectIdMap,
  getGameObjectAnchorOffset,
  getGameObjectTileSize,
} from "../../lib/gameObjectIdMap";
import {
  GameObjectState,
  GamePosition,
  GraphicsObjectState,
...
import {
  computeSolLaserBeams,
  isSolLaserGraphic,
  SolLaserBeam,
} from "../../lib/solLaserBeams";
import {
...
const MapMarkers: React.FC<MapMarkersProps> = ({
  ...
  const replayClientCycle = useMemo(
    () =>
      fightEpochCycle != null
        ? getCurrentGameCycle(currentTime, fightEpochCycle)
        : undefined,
  ...
  const renderedSolLaserBeams = useMemo((): SolLaserRenderData[] => {
    return solLaserBeams.flatMap((beam, index) => {
      ...
      if (definition.frames && definition.frames.length > 0) {
        const frameIndex = getGraphicObjectFrameIndex(
          cyclesElapsed,
          definition,
        );
  ...
  const renderedYamaWalls = useMemo((): YamaWallRenderData[] => {
    ...
    for (const [objectKey, positionData] of Object.entries(
      graphicsObjectPositions,
    )) {
      ...
      walls.push({
        ...
        opacity:
          phase === "active" ? 1 : getYamaShadowWallFadeOpacity(cyclesElapsed),
  ...
  const renderedGraphicsObjects = useMemo((): GraphicsObjectRenderData[] => {
    ...
    for (const [objectKey, positionData] of Object.entries(
      ...
      if (
        isSolLaserGraphic(positionData.id) ||
        isYamaShadowWallGraphic(positionData.id)
      ) {
        continue;
      }
  ...
  const objectHighlights = useMemo((): ObjectHighlightRenderData[] => {
    ...
    for (const [objectKey, positionData] of Object.entries(
      ...
      const rectangle = objectPosition.toLeaflet(
        map,
        getHighlightRectangleOptions("graphics"),
      );
    ...
    for (const [objectKey, positionData] of Object.entries(
      ...
      const rectangle = objectPosition.toLeaflet(
        map,
        getHighlightRectangleOptions("ground"),
      );
  ...
  return (
    <>
      ...
      {Object.entries(npcPositions).map(([npcKey, positionData]) => {
        ...
        const npcPosition = new Position(
          positionData.x,
          positionData.y,
          positionData.plane,
        );
      ...
      {Object.entries(groundObjectPositions).map(
        ([objectKey, positionData]) => {
          if (!isOnMapPlane(positionData.position, plane)) {
            return null;
          }
...
function solLaserBeamToBounds(map: L.Map, beam: SolLaserBeam): LatLngBounds {
  if (beam.orientation === "horizontal") {
    ...
    const northEast = Position.toLatLng(
      map,
      beam.endVar + 1,
      beam.fixedCoord + 1,
    );
