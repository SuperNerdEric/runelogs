...
function getBoostedHitWeight(
  ...
  if (combatClass === CombatClass.Ranged) {
    ...
    boostedHitWeight =
      (levelsBoostedAttack + levelsBoostedStrength) / 2 / meleeBoost;
  }

  return boostedHitWeight;
}

function getWeaponHitsForDuration(
  ...
  const remainingHits = Math.floor(
    durationSeconds / (SECONDS_PER_TICK * currentWeaponSpeed),
  );
 ...
 */
export function getFightPerformanceByPlayer(
  fight: Fight,
): Map<string, FightPerformance> {
  ...
  for (const log of fight.data) {
      ...
      for (const itemId of log.playerEquipment) {
        ...
        if (state.currentWeaponSpeed) {
          const durationSeconds =
            (log.fightTimeMs! - state.expectedLastTimestamp) / 1000;
          const { newWeaponHits, timeTaken } = getWeaponHitsForDuration(
            durationSeconds,
            state.currentWeaponSpeed,
            state.previousWeaponSpeed,
          );
    ...
    if (log.type === LogTypes.PLAYER_ATTACK_ANIMATION) {
      ...
      if (MELEE_ANIMATIONS.includes(log.animationId)) {
        ...
        console.warn(
          `Unknown animation ID: ${log.animationId} for player: ${playerName}`,
        );
        combatClass = CombatClass.Melee; // Fallback to Melee
      }

      if (state.currentWeapon && state.lastBoost) {
        state.boostedHits += getBoostedHitWeight(
          fight,
          combatClass,
          state.currentWeapon,
          state.lastBoost,
      ...
      if (log.fightTimeMs! + weaponSpeedSeconds * 1000 > fightEndMs) {
        weaponSpeedSeconds = Math.max(
          (fightEndMs - log.fightTimeMs!) / 1000,
          0,
        );
      }
  ...
  for (const [playerName, state] of stateByPlayer.entries()) {
    ...
    const { newWeaponHits } = getWeaponHitsForDuration(
      durationSeconds,
      state.currentWeaponSpeed,
      state.previousWeaponSpeed,
    );
