...
export const LEADERBOARD_CONTENT_OPTIONS: LeaderboardContentOption[] = [
  ...
  },
  {
    ...
    value: "Tombs of Amascut",
    spriteKey: "tombs_of_amascut",
  ...
  },
  {
    label: "The Gauntlet",
    value: "The Gauntlet",
    spriteKey: "the_gauntlet",
    playerCounts: [1],
    defaultPlayerCount: 1,
  },
  {
    label: "Corrupted Gauntlet",
    value: "Corrupted Gauntlet",
    spriteKey: "the_corrupted_gauntlet",
    playerCounts: [1],
    defaultPlayerCount: 1,
  },
  {
    label: "Doom of Mokhaiotl",
    value: MOKHAIOTL_CONTENT_NAME,
    spriteKey: "doom_of_mokhaiotl",
    playerCounts: [1],
    defaultPlayerCount: 1,
  },
  {
    label: "Yama",
    value: YAMA_CONTENT_NAME,
    spriteKey: "yama",
    playerCounts: [1, 2],
  ...
  {
    label: "Maggot King",
    value: MAGGOT_KING_CONTENT_NAME,
    spriteKey: "maggot_king",
    playerCounts: [1],
    defaultPlayerCount: 1,
  },
];
...
export function getLeaderboardContentSpriteKey(
  contentValue: string,
): string | undefined {
  if (contentValue === RECENT_ENCOUNTERS_ALL_CONTENT) {
    return "overall";
  }
  return LEADERBOARD_CONTENT_OPTIONS.find(
    (option) => option.value === contentValue,
  )?.spriteKey;
}
...
export const RECENT_ENCOUNTERS_CONTENT_OPTIONS: RecentEncountersContentOption[] =
  [
    {
      label: "All Content",
      value: RECENT_ENCOUNTERS_ALL_CONTENT,
      playerCounts: RECENT_ENCOUNTERS_PARTY_SIZES,
    },
    ...LEADERBOARD_CONTENT_OPTIONS,
  ];

export function resolveRecentEncountersContent(
  ...
  return (
    LEADERBOARD_CONTENT_OPTIONS.find((o) => o.value === contentParam) ??
...
export function browsePlayerCountToApiParam(
...
export function getLeaderboardModeLabel(
  contentName: string,
...
export function resolveLeaderboardContent(
  contentParam: string | null,
): LeaderboardContentOption {
  return (
    LEADERBOARD_CONTENT_OPTIONS.find(
      (option) => option.value === contentParam,
...
export function resolveLeaderboardPlayerCount(
  ...
  const parsed = parseInt(playerCountParam || "", 10);
  return content.playerCounts.includes(parsed)
    ? parsed
...
export function resolveLeaderboardSelectedFight(
  ...
  const fightsForContent =
    dpsConfig.find((group) => group.contentName === contentValue)?.fights ?? [];
  if (
    fightParam &&
    (fightsForContent.length === 0 || fightsForContent.includes(fightParam))
  ) {
    return fightParam;
  }
  if (
    isMokhaiotlLeaderboardContent(contentValue) &&
    fightsForContent.includes(MOKHAIOTL_DELVE_1_8_KEY)
  ) {
...
export function resolveLeaderboardHighlightRank(
  highlightRankParam: string | null,
): number | null {
...
export function resolveLeaderboardStateFromSearchParams(
  ...
  return {
    ...
    content,
    playerCount: resolveLeaderboardPlayerCount(
      content,
    ...
    highlightRank: resolveLeaderboardHighlightRank(
      searchParams.get("highlightRank"),
    ),
  };
}
...
export function isLeaderboardFightGroup(
  leaderboardName: string | null | undefined,
): boolean {
  return Boolean(
    leaderboardName &&
    LEADERBOARD_FIGHT_GROUP_NAMES.includes(
      leaderboardName as (typeof LEADERBOARD_FIGHT_GROUP_NAMES)[number],
    ),
  );
}

export function isStandaloneLeaderboardContent(
  contentName: string | null | undefined,
): boolean {
  return Boolean(
    contentName &&
    STANDALONE_LEADERBOARD_CONTENT_NAMES.includes(
      contentName as (typeof STANDALONE_LEADERBOARD_CONTENT_NAMES)[number],
    ),
  );
}

/** Maps standalone boss fights (no fight group) to their leaderboard content name. */
export function inferStandaloneLeaderboardContent(
  mainEnemyName: string | null | undefined,
): string | null {
  return isStandaloneLeaderboardContent(mainEnemyName) ? mainEnemyName! : null;
}

export function isLeaderboardContent(
  leaderboardName: string | null | undefined,
): boolean {
  return (
    isLeaderboardFightGroup(leaderboardName) ||
