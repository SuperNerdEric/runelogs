...
export function formatFightDurationLabel(name: string, fightDurationTicks: number): string {
...
export function fightCompactRowWidth(tileCount: number, labelCh: number): string {
...
export function fightCompactRowWidthCss(tileCount: number, labelCh: number): string {
    if (tileCount <= 0) {
        return '0px';
    }
...
export function shouldUseCompactFightLayout(
    ...
    return !shouldStretchFightTiles(tileCount, estimateTileMinWidthPx(labelCh), contentWidth);
}
...
export function resolveFamilyTileLayouts(groups: FightGroupTileInput[]): Map<string, FamilyTileLayout> {
