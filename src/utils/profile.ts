...
export type ContactLinkKey =
  "rsn" | "discord" | "twitter" | "youtube" | "twitch" | "kick";
...
function youtubePathContact(path: string): ParsedContact {
  ...
  const display =
    parts.length >= 2 ? parts[parts.length - 1] : (parts[0] ?? normalizedPath);
...
export function getContactDisplayText(
  key: ContactLinkKey,
  value: string,
): string {
  if (key === "rsn") {
...
export function getContactFormValue(
  key: ContactLinkKey,
