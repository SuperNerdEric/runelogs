...
const LEADERBOARD_SEO_SNIPPETS: Record<string, {titleSuffix: string; description: string}> = {
    'Theatre of Blood': {
...
const DEFAULT_LEADERBOARD_META = {
    ...
    description:
        'OSRS combat log leaderboards on Runelogs. Compare run times and boss DPS for Theatre of Blood, Tombs of Amascut, Inferno, Colosseum, Gauntlet, Mokhaiotl, and Yama.',
...
export function getLeaderboardPageMeta(searchParams: URLSearchParams) {
    const {mode, content, playerCount, selectedFight} = resolveLeaderboardStateFromSearchParams(
        searchParams,
        [],
    );
    ...
    const partyText = content.playerCounts.length > 1 ? ` (${playerCount}-player)` : '';
    const fightText = mode === 'dps' && selectedFight && selectedFight !== 'Overall'
        ? ` - ${selectedFight}`
        : '';
