...
export const deserializePrayerFilter = (
  value: string | null,
): PrayerFilter | null => {
  if (!value) {
    return null;
...
export const buildPrayerTimelines = (
  logs: LogLine[],
): Map<string, PrayerSnapshot[]> => {
  const timelines = new Map<string, PrayerSnapshot[]>();
  const currentState = new Map<
    string,
    { prayers: string[]; overhead?: string }
  >();
...
export const playerHasPrayerAtTime = (
  ...
  if (
    state.prayers.some((prayerIdStr) => parseInt(prayerIdStr, 10) === prayerId)
  ) {
    return true;
  }
