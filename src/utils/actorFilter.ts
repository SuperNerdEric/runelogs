...
export const deserializeActorFilter = (value: string | null): ActorFilter | null => {
    if (!value) {
        return null;
    }
...
export const matchesActorFilter = (actor: Actor | undefined, filter: ActorFilter | null): boolean => {
    if (!filter) {
        return true;
    }
    if (!actor) {
...
export const matchesLogActorFilters = (
    ...
    return matchesActorFilter(sourceActor, sourceFilter) && matchesActorFilter(targetActor, targetFilter);
};
