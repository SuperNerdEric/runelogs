...
export function combineOverallUploadProgress(
    ...
    const uploadSlice = (Math.min(100, Math.max(0, uploadPercent)) / 100) * UPLOAD_FILE_OVERALL_WEIGHT;
    if (parsePercent === null) {
        return uploadSlice;
    }
    const parseSlice = (Math.min(100, Math.max(0, parsePercent)) / 100) * PARSE_OVERALL_WEIGHT;
