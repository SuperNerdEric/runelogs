...
export function getFightDurationSeconds(fight: Fight): number {
    ...
    return Math.max((fight.lastLine.fightTimeMs ?? 0) - (fight.firstLine.fightTimeMs ?? 0), 0) / 1000;
}
...
export function isLeaderboardEligibleDamage(damageLog: DamageLog): boolean {
    return Boolean(damageLog.target?.index) &&
        (damageLog.target?.id === undefined || !BOAT_IDS.includes(damageLog.target.id));
}
 ...
 */
export function calculatePlayerDps(fight: Fight, logs?: Iterable<LogLine>): PlayerDpsResult[] {
    ...
    for (const logLine of sourceLogs) {
        ...
        damageByPlayer.set(sourceName, (damageByPlayer.get(sourceName) ?? 0) + damageLog.damageAmount);
    }
    ...
    return fight.players.map((playerName) => {
