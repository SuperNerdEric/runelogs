...
export const deserializeEquipmentFilter = (value: string | null): EquipmentFilter | null => {
    if (!value) {
        return null;
    }
...
export const buildEquipmentTimelines = (logs: LogLine[]): Map<string, EquipmentSnapshot[]> => {
    const timelines = new Map<string, EquipmentSnapshot[]>();

    for (const log of logs) {
        if (log.type !== LogTypes.PLAYER_EQUIPMENT || !log.source?.name || !log.playerEquipment) {
            continue;
        }
...
export const matchesEquipmentFilter = (
    ...
    if (playersToCheck.size === 0) {
        for (const playerName of timelines.keys()) {
            if (playerHasEquipmentAtTime(timelines, playerName, timeMs, equipmentFilter.id)) {
                return true;
            }
        }
        return false;
