...
test.describe('Log page', () => {
    ...
    test('info box does not overflow horizontally on a narrow viewport', async ({page}) => {
        ...
        const overflow = await page.evaluate(() => {
            ...
            const childOverflow = box
                ? [...box.children].some((el) => el.getBoundingClientRect().right > (boxRect?.right ?? 0) + 1)
                : false;
    ...
    test('fight selector does not overflow horizontally on a narrow viewport', async ({page}) => {
        await page.setViewportSize({width: 357, height: 667});
        await page.goto(`/log/${testLogId}`);

        const fightSelector = page.locator('.fight-selector');
        await expect(fightSelector).toBeVisible();
        await expect(fightSelector.getByText('The Maiden of Sugadinti')).toBeVisible();
        ...
        const overflow = await page.evaluate(() => {
            ...
            const tileOverflow = [...document.querySelectorAll('.fight-tile')].some((tile) => {
