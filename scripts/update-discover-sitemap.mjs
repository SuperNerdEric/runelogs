...
const leaderboardContentPath = join(
  __dirname,
  "..",
  "src",
  "utils",
  "leaderboardContent.ts",
);
...
function parseLeaderboardContentOptions() {
  ...
  if (!optionsBlock) {
    throw new Error(
      "LEADERBOARD_CONTENT_OPTIONS not found in leaderboardContent.ts",
    );
  }
  ...
  for (const match of optionsBlock[1].matchAll(optionPattern)) {
    ...
    if (!value) {
      throw new Error(
        `Unable to resolve leaderboard content value: ${match[2] ?? match[1]}`,
      );
    }
  ...
  if (options.length === 0) {
    throw new Error(
      "No leaderboard content options parsed from leaderboardContent.ts",
    );
  }
...
function replaceDiscoverSection(sitemap, discoverXml) {
  const pattern =
    / {4}<!-- generated:discover -->[\s\S]*? {4}<!-- \/generated:discover -->/;
...
function updateDiscoverSitemap() {
  ...
  const recentEncountersUrls = buildRecentEncountersSitemapUrls(
    SITE_URL,
    contentOptions,
  );
  const discoverXml = buildDiscoverSitemapXml({
    leaderboardUrls,
    recentEncountersUrls,
  });
